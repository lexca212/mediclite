jQuery().ready(function () {
    var var_tbl_mlite_antrian_referensi_taskid = $('#tbl_mlite_antrian_referensi_taskid').DataTable({
        ...
        "bInfo" : false,
        "ajax": {
            "url": "{?=url([ADMIN,'mlite_antrian_referensi_taskid','data'])?}",
            "dataType": "json",
            "type": "POST",
            "data": function (data) {
...
var sep= $('#sep').val();
 ...
 formData.append('typeact', typeact); // tambahan
            ...
            $.ajax({
                url: "{?=url([ADMIN,'mlite_antrian_referensi_taskid','aksi'])?}",
                method: "POST",
...
$('#sep').val(sep);
            ...
            $('#modal-title').text("Edit Data mlite antrian referensi taskid");
...
$('#sep').val('');
        ...
        $('#modal-title').text("Tambah Data mlite antrian referensi taskid");
        $("#modal_mlite_antrian_referensi_taskid").modal();
    });
    ...
    $("#lihat_data_mlite_antrian_referensi_taskid").click(function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                var eTable = "<div class='table-responsive'><table id='tbl_lihat_mlite_antrian_referensi_taskid' class='table display dataTable' style='width:100%'><thead><th>Tanggal Periksa</th><th>Nomor Referensi</th><th>Taskid</th><th>Waktu</th><th>Status</th><th>Keterangan</th><th>Sep</th></thead>";
...
var tanggal_periksa = rowData['tanggal_periksa'];
            ...
            var loadURL =  baseURL + '/mlite_antrian_referensi_taskid/detail/' + tanggal_periksa + '?t=' + mlite.token;

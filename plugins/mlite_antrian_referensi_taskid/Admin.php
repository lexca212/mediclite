<?php
namespace Plugins\mlite_antrian_referensi_taskid;
...
$sep = $_POST['sep'];
            ...
            $mlite_antrian_referensi_taskid_add->execute([$tanggal_periksa, $nomor_referensi, $taskid, $waktu, $status, $keterangan, $sep]);
...
$sep = $_POST['sep'];
        ...
        // BUANG FIELD PERTAMA
            ...
            $mlite_antrian_referensi_taskid_edit = $this->db()->pdo()->prepare("UPDATE mlite_antrian_referensi_taskid SET tanggal_periksa=?, nomor_referensi=?, taskid=?, waktu=?, status=?, keterangan=?, sep=? WHERE tanggal_periksa=?");
            $mlite_antrian_referensi_taskid_edit->execute([$tanggal_periksa, $nomor_referensi, $taskid, $waktu, $status, $keterangan, $sep,$tanggal_periksa]);
        ...
        if ($act=="lihat") {
            ...
            $user_lihat = $this->db()->pdo()->prepare("SELECT * from mlite_antrian_referensi_taskid WHERE 1 ".$searchQuery);
            $user_lihat->execute();
...
'sep'=>$row['sep']
    ...
    public function getCss()
    {
        ...
        echo $this->draw(MODULES.'/mlite_antrian_referensi_taskid/css/admin/styles.css');
        exit();
    }
    ...
    public function getJavascript()
    {
        ...
        echo $this->draw(MODULES.'/mlite_antrian_referensi_taskid/js/admin/scripts.js', ['settings' => $settings]);
        exit();
    }
    ...
    private function _addHeaderFiles()
    {
        ...
        $this->core->addCSS(url([ADMIN, 'mlite_antrian_referensi_taskid', 'css']));
        $this->core->addJS(url([ADMIN, 'mlite_antrian_referensi_taskid', 'javascript']), 'footer');
    }

...
class Admin extends AdminModule
{
  ...
  private function _getSEPInfo($field, $no_rawat)
  {
    // cari SEP rawat inap dulu
    $row = $this->db('bridging_sep')
                ->where('no_rawat', $no_rawat)
                ->where('jnspelayanan', '1') // 1 = rawat inap
                ->desc('tglrujukan')
                ->oneArray();

    // kalau tidak ada rawat inap, ambil rawat jalan terakhir
    if(!$row) {
        $row = $this->db('bridging_sep')
                    ->where('no_rawat', $no_rawat)
                    //->desc('tglrujukan')
  ...
  public function getBridgingInacbgs($no_rawat)
  {
    ...
    if($reg_periksa['status_lanjut'] == 'Ranap') {
      $pemeriksaan = $this->db('pemeriksaan_ranap')->where('no_rawat', $reg_periksa['no_rawat'])->limit(1)->asc('tgl_perawatan')->asc('jam_rawat')->toArray();
      $reg_periksa['sistole'] = strtok($pemeriksaan[0]['tensi'], '/');
      $reg_periksa['diastole'] = substr($pemeriksaan[0]['tensi'], strpos($pemeriksaan[0]['tensi'], '/') + 1);
    }
    ...
    $biaya_konsultasi_jl_dr = $this->db('rawat_jl_dr')
      ->select(['biaya_rawat' => 'SUM(biaya_rawat)'])
      ->join('jns_perawatan', 'jns_perawatan.kd_jenis_prw=rawat_jl_dr.kd_jenis_prw')
    ...
    $biaya_konsultasi_jl_pr = $this->db('rawat_jl_pr')
      ->select(['biaya_rawat' => 'SUM(biaya_rawat)'])
      ->join('jns_perawatan', 'jns_perawatan.kd_jenis_prw=rawat_jl_pr.kd_jenis_prw')
      ->where('jns_perawatan.kd_kategori', $this->settings->get('vedika.inacbgs_konsultasi'))
      ->where('no_rawat', revertNoRawat($no_rawat))
    ...
    $biaya_konsultasi_jl_drpr = $this->db('rawat_jl_drpr')
      ->select(['biaya_rawat' => 'SUM(biaya_rawat)'])
      ->join('jns_perawatan', 'jns_perawatan.kd_jenis_prw=rawat_jl_drpr.kd_jenis_prw')
      ->where('jns_perawatan.kd_kategori', $this->settings->get('vedika.inacbgs_konsultasi'))
      ->where('no_rawat', revertNoRawat($no_rawat))
    ...
    $biaya_konsultasi_inap_dr = $this->db('rawat_inap_dr')
      ->select(['biaya_rawat' => 'SUM(biaya_rawat)'])
      ->join('jns_perawatan_inap', 'jns_perawatan_inap.kd_jenis_prw=rawat_inap_dr.kd_jenis_prw')
    ...
    $biaya_konsultasi_inap_pr = $this->db('rawat_inap_pr')
      ->select(['biaya_rawat' => 'SUM(biaya_rawat)'])
      ->join('jns_perawatan_inap', 'jns_perawatan_inap.kd_jenis_prw=rawat_inap_pr.kd_jenis_prw')
      ->where('jns_perawatan_inap.kd_kategori', $this->settings->get('vedika.inacbgs_konsultasi'))
      ->where('no_rawat', revertNoRawat($no_rawat))
    ...
    $biaya_konsultasi_inap_drpr = $this->db('rawat_inap_drpr')
      ->select(['biaya_rawat' => 'SUM(biaya_rawat)'])
      ->join('jns_perawatan_inap', 'jns_perawatan_inap.kd_jenis_prw=rawat_inap_drpr.kd_jenis_prw')
      ->where('jns_perawatan_inap.kd_kategori', $this->settings->get('vedika.inacbgs_konsultasi'))
      ->where('no_rawat', revertNoRawat($no_rawat))
    ...
    foreach (array_merge( $biaya_konsultasi_jl_dr, $biaya_konsultasi_jl_pr, $biaya_konsultasi_jl_drpr,$biaya_konsultasi_inap_dr, $biaya_konsultasi_inap_pr, $biaya_konsultasi_inap_drpr) as $row) {
      $total_biaya_konsultasi += $row['biaya_rawat'];
    }

    /* Biaya Tenaga Ahli */
    $biaya_tenaga_ahli_inap_dr = $this->db('rawat_inap_dr')
      ->select(['biaya_rawat' => 'SUM(biaya_rawat)'])
      ->join('jns_perawatan_inap', 'jns_perawatan_inap.kd_jenis_prw=rawat_inap_dr.kd_jenis_prw')
    ...
    $biaya_tenaga_ahli_inap_pr = $this->db('rawat_inap_pr')
      ->select(['biaya_rawat' => 'SUM(biaya_rawat)'])
      ->join('jns_perawatan_inap', 'jns_perawatan_inap.kd_jenis_prw=rawat_inap_pr.kd_jenis_prw')
      ->where('jns_perawatan.kd_kategori', $this->settings->get('vedika.inacbgs_tenaga_ahli'))
      ->where('no_rawat', revertNoRawat($no_rawat))
    ...
    $biaya_tenaga_ahli_inap_drpr = $this->db('rawat_inap_drpr')
      ->select(['biaya_rawat' => 'SUM(biaya_rawat)'])
      ->join('jns_perawatan_inap', 'jns_perawatan_inap.kd_jenis_prw=rawat_inap_drpr.kd_jenis_prw')
      ->where('jns_perawatan.kd_kategori', $this->settings->get('vedika.inacbgs_tenaga_ahli'))
      ->where('no_rawat', revertNoRawat($no_rawat))
    ...
    $biaya_tenaga_ahli_jl_dr = $this->db('rawat_jl_dr')
      ->select(['biaya_rawat' => 'SUM(biaya_rawat)'])
      ->join('jns_perawatan', 'jns_perawatan.kd_jenis_prw=rawat_inap_dr.kd_jenis_prw')
      ->where('jns_perawatan.kd_kategori', $this->settings->get('vedika.inacbgs_tenaga_ahli'))
      ->where('no_rawat', revertNoRawat($no_rawat))
    ...
    $biaya_tenaga_ahli_jl_pr = $this->db('rawat_jl_pr')
      ->select(['biaya_rawat' => 'SUM(biaya_rawat)'])
      ->join('jns_perawatan', 'jns_perawatan.kd_jenis_prw=rawat_inap_pr.kd_jenis_prw')
      ->where('jns_perawatan.kd_kategori', $this->settings->get('vedika.inacbgs_tenaga_ahli'))
      ->where('no_rawat', revertNoRawat($no_rawat))
    ...
    $biaya_tenaga_ahli_jl_drpr = $this->db('rawat_jl_drpr')
      ->select(['biaya_rawat' => 'SUM(biaya_rawat)'])
      ->join('jns_perawatan', 'jns_perawatan.kd_jenis_prw=rawat_inap_drpr.kd_jenis_prw')
      ->where('jns_perawatan.kd_kategori', $this->settings->get('vedika.inacbgs_tenaga_ahli'))
      ->where('no_rawat', revertNoRawat($no_rawat))
    ...
    */
    ...
    $ppnralan = $total_biaya_obat * 11 / 100 + $total_biaya_obat;
    $tampilppn = intval($ppnralan);
    
    if($reg_periksa['status_lanjut'] == 'Ralan') {
      $pemeriksaan = $this->db('pemeriksaan_ralan')->where('no_rawat', $reg_periksa['no_rawat'])->limit(1)->desc('tgl_perawatan')->desc('jam_rawat')->toArray();
      $reg_periksa['tgl_registrasi'] = $pemeriksaan[0]['tgl_perawatan'];
      $reg_periksa['jam_reg'] = $pemeriksaan[0]['jam_rawat'];
      //mulai jam keluar disini

      // $datetime_reg = $reg_periksa['tgl_registrasi'] . ' ' . $reg_periksa['jam_reg'];

      // $jam_keluar = date(
      //   'H:i:s',
      //   strtotime($datetime_reg . ' +45 minutes')
      // );

      // $reg_periksa['jam_keluar'] = $jam_keluar;
    ...
    echo $this->draw('inacbgs.html', [
      ...
      'biaya_obat' => ($reg_periksa['status_lanjut'] == 'Ralan') ? $tampilppn : $total_biaya_obat,

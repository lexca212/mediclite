...
class Admin extends AdminModule
{
  ...
  public function _Display($tgl_masuk = '', $tgl_masuk_akhir = '', $status_pulang = '', $status_periksa = '')
  {
    ...
    if ($this->core->getUserInfo('role') != 'admin') {
      ...
      $sql_rg .= " AND dpjp_ranap.kd_dokter = '$username'"; // --- ADD FILTER FOR RANAP GABUNG
    }
    if ($status_pulang == '') {
    ...
    if ($status_periksa == 'lunas' && $status_pulang == '-' && $tgl_masuk !== '' && $tgl_masuk_akhir !== '') {
      ...
      $sql_rg .= " AND reg_periksa.status_bayar = 'Sudah Bayar' AND kamar_inap.tgl_masuk BETWEEN '$tgl_masuk' AND '$tgl_masuk_akhir'"; // --- ADD FILTER FOR RANAP GABUNG
    }

    // --- MERGE BOTH SQL QUERIES ---
    $sql_final = $sql . " UNION " . $sql_rg;

    $sql_final .= " GROUP BY reg_periksa.no_rawat";
  ...
  //======================
  //============LembarMasukICu======/
  public function getMasukNicu($no_rawat)
  {
    ...
    $this->tpl->set('masuk_nicu', $this->db('mlite_masuk_nicu')->where('no_rawat', revertNoRawat($no_rawat))->join('dokter', 'dokter.kd_dokter=mlite_masuk_nicu.kd_dokter')->oneArray());
    echo $this->tpl->draw(MODULES . '/dokter_ranap/view/admin/masuk.nicu.html', true);
    exit();
  }
  ...
  {
    $data_medisRanap['pemeriksaan_ranap'] = $this->db('pemeriksaan_ranap')->where('no_rawat', revertNoRawat($no_rawat))->oneArray();
    echo $this->draw('medis.ranap.html', [
      'reg_periksa' => $this->db('reg_periksa')->where('no_rawat', revertNoRawat($no_rawat))->oneArray(),
      'penilaian_medis_ranap' => $this->db('penilaian_medis_ranap')->where('no_rawat', revertNoRawat($no_rawat))->join('dokter', 'dokter.kd_dokter=penilaian_medis_ranap.kd_dokter')->oneArray(),
      'pasien'  => $this->db('pasien')->where('no_rawat', revertNoRawat($no_rawat))->join('reg_periksa', 'pasien.no_rkm_medis=reg_periksa.no_rkm_medis')->oneArray(),
      ...
      'data_medisRanap' => $data_medisRanap
    ]);
    exit();
  }
  ...
  {
    echo $this->draw('medis.ranap.tampil.html', ['penilaian_medis_ranap' => $this->db('penilaian_medis_ranap')->where('no_rawat', revertNoRawat($no_rawat))->join('dokter', 'dokter.kd_dokter=penilaian_medis_ranap.kd_dokter')->oneArray()]);
    exit();
  }
  ...
  public function postMedisRanap()
  {
    ...
    if ($this->db('penilaian_medis_ranap')->where('no_rawat', $_POST['no_rawat'])->where('kd_dokter', $_POST['kd_dokter'])->oneArray()) {
  ...
  // Start Lembar Keluar ICU


  public function getLembarKeluarIcu($no_rawat)
  {
    ...
    echo $this->draw('lembar.keluar.icu.html', [
        'pasien'           => $this->db('pasien')
                                ->where('no_rawat', revertNoRawat($no_rawat))
  ...
  public function postLembarKeluarIcu()
  {
    $_POST['kd_dokter'] = $this->core->getUserInfo('username', $_SESSION['mlite_user']);

    if ($this->db('kriteria_keluar_icu')->where('no_rawat', $_POST['no_rawat'])->where('kd_dokter', $_POST['kd_dokter'])->oneArray()) {
      $this->db('kriteria_keluar_icu')
        ->where('no_rawat', $_POST['no_rawat'])
        ->save([
          //'no_rawat' => $_POST['no_rawat'],
          ...
          'diagnosa'    => $_POST['diagnosa'],
          'kd_dokter'   => $_POST['kd_dokter'],
          'dokter_konsultan'  => $_POST['dokter_konsultan'],
          'lainnya'     => $_POST['lainnya'],
          'kesadaran'    => json_encode($_POST['kesadaran'] ?? []),
          'jalan_nafas'  => json_encode($_POST['jalan_nafas'] ?? []),
          'pernapasan'   => json_encode($_POST['pernapasan'] ?? []),
          'sirkulasi'    => json_encode($_POST['sirkulasi'] ?? []),
          'kondisi_lain' => json_encode($_POST['kondisi_lain'] ?? []),
        ]);
    } else {
      $this->db('kriteria_keluar_icu')->save([
        'no_rawat' => $_POST['no_rawat'],
        'tanggal_jam' => $_POST['tanggal_jam'],
        'diagnosa'    => $_POST['diagnosa'],
        'kd_dokter'   => $_POST['kd_dokter'],

...
class Admin extends AdminModule
{
  ...
  public function getHasilusg($no_rawat)
    {
      $data_hasilusg['pemeriksaan_ralan'] = $this->db('pemeriksaan_ralan')->where('no_rawat', revertNoRawat($no_rawat))->oneArray();
      echo $this->draw('hasil.usg.html', [
        'reg_periksa' => $this->db('reg_periksa')->where('no_rawat', revertNoRawat($no_rawat))->oneArray(),
        'hasil_pemeriksaan_usg' => $this->db('hasil_pemeriksaan_usg')->where('no_rawat', revertNoRawat($no_rawat))->join('dokter', 'dokter.kd_dokter=hasil_pemeriksaan_usg.kd_dokter')->oneArray(),
        ...
        'data_hasilusg' => $data_hasilusg
      ]);
      exit();
    }
    public function getHasilusgTampil($no_rawat)
    {
      echo $this->draw('hasil.usg.tampil.html', ['hasil_pemeriksaan_usg' => $this->db('hasil_pemeriksaan_usg')->where('no_rawat', revertNoRawat($no_rawat))->join('dokter', 'dokter.kd_dokter=hasil_pemeriksaan_usg.kd_dokter')->oneArray()]);
      exit();
    }
    public function postHasilusg()
    {
      $_POST['kd_dokter'] = $this->core->getUserInfo('username', $_SESSION['mlite_user']);

      if($this->db('hasil_pemeriksaan_usg')->where('no_rawat', $_POST['no_rawat'])->where('kd_dokter', $_POST['kd_dokter'])->oneArray()) {
        $this->db('hasil_pemeriksaan_usg')
          ->where('no_rawat', $_POST['no_rawat'])
          ->save([
          'kd_dokter'                 =>  $_POST['kd_dokter'],
          'tanggal'                   =>  $_POST['tanggal'],  
          'diagnosa_klinis'           =>  $_POST['diagnosa_klinis'],    
          'kiriman_dari'              =>  $_POST['kiriman_dari'],    
          'hta'                       =>  $_POST['hta'],    
          'kantong_gestasi'           =>  $_POST['kantong_gestasi'],    
          'ukuran_bokongkepala'       =>  $_POST['ukuran_bokongkepala'],    
          'jenis_prestasi'            =>  $_POST['jenis_prestasi'],    
          'diameter_biparietal'       =>  $_POST['diameter_biparietal'],    
          'panjang_femur'             =>  $_POST['panjang_femur'],    
          'lingkar_abdomen'           =>  $_POST['lingkar_abdomen'],    
          'tafsiran_berat_janin'      =>  $_POST['tafsiran_berat_janin'],    
          'usia_kehamilan'            =>  $_POST['usia_kehamilan'],    
          'plasenta_berimplatansi'    =>  $_POST['plasenta_berimplatansi'],    
          'derajat_maturitas'         =>  $_POST['derajat_maturitas'],    
          'jumlah_air_ketuban'        =>  $_POST['jumlah_air_ketuban'],    
          'indek_cairan_ketuban'      =>  $_POST['indek_cairan_ketuban'],    
          'kelainan_kongenital'       =>  $_POST['kelainan_kongenital'],    
          'peluang_sex'                =>  $_POST['peluang_sex'],    
          'kesimpulan'                =>  $_POST['kesimpulan']
          ]);
        } else {
          $this->db('hasil_pemeriksaan_usg')->save([
          'no_rawat'                  => $_POST['no_rawat'],
          'kd_dokter'                 =>  $_POST['kd_dokter'],
          'tanggal'                   =>  $_POST['tanggal'],  
          'diagnosa_klinis'           =>  $_POST['diagnosa_klinis'],    
    ...
    public function getMedisAnak($no_rawat)
    {
      $data_medisAnak['pemeriksaan_ralan'] = $this->db('pemeriksaan_ralan')->where('no_rawat', revertNoRawat($no_rawat))->oneArray();
      echo $this->draw('medis.anak.html', [
        'reg_periksa' => $this->db('reg_periksa')->where('no_rawat', revertNoRawat($no_rawat))->oneArray(),
        'penilaian_medis_ralan_anak' => $this->db('penilaian_medis_ralan_anak')->where('no_rawat', revertNoRawat($no_rawat))->join('dokter', 'dokter.kd_dokter=penilaian_medis_ralan_anak.kd_dokter')->oneArray(),
        'pasien'  => $this->db('pasien')->where('no_rawat', revertNoRawat($no_rawat))->join('reg_periksa','pasien.no_rkm_medis=reg_periksa.no_rkm_medis')->oneArray(),
        'dokter'  => $this->db('dokter')->where('no_rawat', revertNoRawat($no_rawat))->join('reg_periksa','dokter.kd_dokter=reg_periksa.kd_dokter')->oneArray(),
        'data_medisAnak' => $data_medisAnak
      ]);
      exit();
    }

    public function getMedisAnakTampil($no_rawat)
    {
      echo $this->draw('medis.anak.tampil.html', ['penilaian_medis_ralan_anak' => $this->db('penilaian_medis_ralan_anak')->where('no_rawat', revertNoRawat($no_rawat))->join('dokter', 'dokter.kd_dokter=penilaian_medis_ralan_anak.kd_dokter')->oneArray()]);
      exit();
    }

    public function postMedisAnak()
    {
      $_POST['kd_dokter'] = $this->core->getUserInfo('username', $_SESSION['mlite_user']);

      if($this->db('penilaian_medis_ralan_anak')->where('no_rawat', $_POST['no_rawat'])->where('kd_dokter', $_POST['kd_dokter'])->oneArray()) {
        $this->db('penilaian_medis_ralan_anak')
          ->where('no_rawat', $_POST['no_rawat'])
          ->save([
          'kd_dokter'           =>  $_POST['kd_dokter'],
          'tanggal'             =>  $_POST['tanggal'],  
        ...
        $this->db('penilaian_medis_ralan_anak')->save([
          'no_rawat'            => $_POST['no_rawat'],
          'kd_dokter'           => $_POST['kd_dokter'],
          'tanggal'             =>  $_POST['tanggal'],    
          'anamnesis'           =>  $_POST['anamnesis'],    
    ...
    public function getMedisKandungan($no_rawat)
    {
      $data_medisKandungan['pemeriksaan_ralan'] = $this->db('pemeriksaan_ralan')->where('no_rawat', revertNoRawat($no_rawat))->oneArray();
      echo $this->draw('medis.kandungan.html', [
        'reg_periksa' => $this->db('reg_periksa')->where('no_rawat', revertNoRawat($no_rawat))->oneArray(),
        'penilaian_medis_ralan_kandungan' => $this->db('penilaian_medis_ralan_kandungan')->where('no_rawat', revertNoRawat($no_rawat))->join('dokter', 'dokter.kd_dokter=penilaian_medis_ralan_kandungan.kd_dokter')->oneArray(),
        'pasien'  => $this->db('pasien')->where('no_rawat', revertNoRawat($no_rawat))->join('reg_periksa','pasien.no_rkm_medis=reg_periksa.no_rkm_medis')->oneArray(),
        'dokter'  => $this->db('dokter')->where('no_rawat', revertNoRawat($no_rawat))->join('reg_periksa','dokter.kd_dokter=reg_periksa.kd_dokter')->oneArray(),
        'data_medisKandungan' => $data_medisKandungan
      ]);
      exit();
    }

    public function getMedisKandunganTampil($no_rawat)
    {
      echo $this->draw('medis.kandungan.tampil.html', ['penilaian_medis_ralan_kandungan' => $this->db('penilaian_medis_ralan_kandungan')->where('no_rawat', revertNoRawat($no_rawat))->join('dokter', 'dokter.kd_dokter=penilaian_medis_ralan_kandungan.kd_dokter')->oneArray()]);
      exit();
    }

    public function postMedisKandungan()
    {
      $_POST['kd_dokter'] = $this->core->getUserInfo('username', $_SESSION['mlite_user']);

      if($this->db('penilaian_medis_ralan_kandungan')->where('no_rawat', $_POST['no_rawat'])->where('kd_dokter', $_POST['kd_dokter'])->oneArray()) {
        $this->db('penilaian_medis_ralan_kandungan')
          ->where('no_rawat', $_POST['no_rawat'])
          ->save([
          'kd_dokter'           =>  $_POST['kd_dokter'],
          'tanggal'             =>  $_POST['tanggal'],  
        ...
        $this->db('penilaian_medis_ralan_kandungan')->save([
          'no_rawat'            => $_POST['no_rawat'],
          'kd_dokter'           => $_POST['kd_dokter'],
          'tanggal'             =>  $_POST['tanggal'],    
          'anamnesis'           =>  $_POST['anamnesis'],    
...
// surt rujukan versi saya
    ...
    // Cek apakah `no_rawat` sudah ada di database
    if ($this->db('mlite_surat_rujukan')->where('no_rawat', $_POST['no_rawat'])->oneArray()) {
        // Jika ada → UPDATE
        $query = $this->db('mlite_surat_rujukan')
            ->where('no_rawat', $_POST['no_rawat'])
            ->save($data);
    } else {
        ...
        $query = $this->db('mlite_surat_rujukan')->save($data);
    }
    ...
    if ($query) {
        ...
        echo json_encode([
            'status' => 'error',
            'msg'    => (is_object($query) && method_exists($query, 'errorInfo'))
                        ? $query->errorInfo()['2']
                        : 'Unknown error'
        ]);
    }

jQuery().ready(function () {
    var var_tbl_penilaian_medis_igd = $('#tbl_penilaian_medis_igd').DataTable({
        'processing': true,
        'serverSide': true,
        'serverMethod': 'post',
        'dom': 'Bfrtip',
        ...
        "bInfo" : false,
        "ajax": {
            "url": "{?=url([ADMIN,'medis_igd_khanza','data'])?}",
            "dataType": "json",
            "type": "POST",
            "data": function (data) {

                // Read values
                var search_field_penilaian_medis_igd = $('#search_field_penilaian_medis_igd').val();
                var search_text_penilaian_medis_igd = $('#search_text_penilaian_medis_igd').val();
                
                data.search_field_penilaian_medis_igd = search_field_penilaian_medis_igd;
                data.search_text_penilaian_medis_igd = search_text_penilaian_medis_igd;
...
{ 'targets': 35}
    ...
    $("form[name='form_penilaian_medis_igd']").validate({
        rules: {
no_rawat: 'required',
tanggal: 'required',
kd_dokter: 'required',
...
mata:'mata tidak boleh kosong!',
gigi:'gigi tidak boleh kosong!',
leher:'leher tidak boleh kosong!',
...
ekg:'ekg tidak boleh kosong!',
rad:'rad tidak boleh kosong!',
lab:'lab tidak boleh kosong!',
...
var tata= $('#tata').val();
 ...
 formData.append('typeact', typeact); // tambahan
            ...
            $.ajax({
                url: "{?=url([ADMIN,'medis_igd_khanza','aksi'])?}",
                method: "POST",
    ...
    // ==============================================================
    $('#search_text_penilaian_medis_igd').keyup(function () {
        var_tbl_penilaian_medis_igd.draw();
    });
    // ==============================================================
    // CLICK TANDA X DI INPUT SEARCH
    // ==============================================================
    $("#searchclear_penilaian_medis_igd").click(function () {
        $("#search_text_penilaian_medis_igd").val("");
        var_tbl_penilaian_medis_igd.draw();
    });
    ...
    $("#edit_data_penilaian_medis_igd").click(function () {
        var rowData = var_tbl_penilaian_medis_igd.rows({ selected: true }).data()[0];
        if (rowData != null) {
...
var mata = rowData['mata'];
var gigi = rowData['gigi'];
var leher = rowData['leher'];
...
$('#tata').val(tata);
            ...
            $('#modal-title').text("Edit Data medis igd khanza");
            $("#modal_penilaian_medis_igd").modal();
        }
        else {
    ...
    // ==============================================================
    jQuery("#hapus_data_penilaian_medis_igd").click(function () {
        var rowData = var_tbl_penilaian_medis_igd.rows({ selected: true }).data()[0];
...
var no_rawat = rowData['no_rawat'];
    ...
    // ==============================================================
    jQuery("#tambah_data_penilaian_medis_igd").click(function () {
...
$('#tata').val('');
        ...
        $('#modal-title').text("Tambah Data medis igd khanza");
        $("#modal_penilaian_medis_igd").modal();
    });
    ...
    // ===========================================
    $("#lihat_data_penilaian_medis_igd").click(function () {
        ...
        $.ajax({
            ...
            data: {
                typeact: 'lihat', 
                search_field_penilaian_medis_igd: search_field_penilaian_medis_igd, 
                search_text_penilaian_medis_igd: search_text_penilaian_medis_igd
            },
            dataType: 'json',
            success: function (res) {
                var eTable = "<div class='table-responsive'><table id='tbl_lihat_penilaian_medis_igd' class='table display dataTable' style='width:100%'><thead><th>No Rawat</th><th>Tanggal</th><th>Kd Dokter</th><th>Anamnesis</th><th>Hubungan</th><th>Keluhan Utama</th><th>Rps</th><th>Rpd</th><th>Rpk</th><th>Rpo</th><th>Alergi</th><th>Keadaan</th><th>Gcs</th><th>Kesadaran</th><th>Td</th><th>Nadi</th><th>Rr</th><th>Suhu</th><th>Spo</th><th>Bb</th><th>Tb</th><th>Kepala</th><th>Mata</th><th>Gigi</th><th>Leher</th><th>Thoraks</th><th>Abdomen</th><th>Genital</th><th>Ekstremitas</th><th>Ket Fisik</th><th>Ket Lokalis</th><th>Ekg</th><th>Rad</th><th>Lab</th><th>Diagnosis</th><th>Tata</th></thead>";
...
eTable += '<td>' + res[i]['mata'] + '</td>';
eTable += '<td>' + res[i]['gigi'] + '</td>';
eTable += '<td>' + res[i]['leher'] + '</td>';
...
eTable += '<td>' + res[i]['ekg'] + '</td>';
eTable += '<td>' + res[i]['rad'] + '</td>';
eTable += '<td>' + res[i]['lab'] + '</td>';
eTable += '<td>' + res[i]['diagnosis'] + '</td>';
eTable += '<td>' + res[i]['tata'] + '</td>';
                    eTable += "</tr>";
                }
                eTable += "</tbody></table></div>";
                $('#forTable_penilaian_medis_igd').html(eTable);
            }
        });

        $('#modal-title').text("Lihat Data");
        $("#modal_lihat_penilaian_medis_igd").modal();
    });

    // ==============================================================
    // TOMBOL DETAIL penilaian_medis_igd DI CLICK
    // ==============================================================
    jQuery("#lihat_detail_penilaian_medis_igd").click(function (event) {
...
var no_rawat = rowData['no_rawat'];
            ...
            var loadURL =  baseURL + '/medis_igd_khanza/detail/' + no_rawat + '?t=' + mlite.token;
        
            var modal = $('#modal_detail_penilaian_medis_igd');
            var modalContent = $('#modal_detail_penilaian_medis_igd .modal-content');
    ...
    $("#export_pdf").click(function () {
        ...
        doc.setFontSize(14);
        doc.text("Tabel Data Penilaian Medis Igd", 20, 95, null, null, null);
        const totalPagesExp = "{total_pages_count_string}";        
        doc.autoTable({
            html: '#tbl_lihat_penilaian_medis_igd',
            startY: 105,
            margin: {
                left: 20, 
                right: 20
        ...
        // doc.save('table_data_penilaian_medis_igd.pdf')
    ...
    $("#export_xlsx").click(function () {
        let tbl1 = document.getElementById("tbl_lihat_penilaian_medis_igd");
        let worksheet_tmp1 = XLSX.utils.table_to_sheet(tbl1);
        let a = XLSX.utils.sheet_to_json(worksheet_tmp1, { header: 1 });
        let worksheet1 = XLSX.utils.json_to_sheet(a, { skipHeader: true });
        const new_workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(new_workbook, worksheet1, "Data penilaian_medis_igd");

...
class Admin extends AdminModule
{
    ...
    public function getPersetujuanRanap($no_rawat)
    {
      $settings = $this->settings('settings');
      $this->tpl->set('settings', $this->tpl->noParse_array(htmlspecialchars_array($settings)));
      ...
      $pasien[0]['jk_text'] = ($pasien[0]['jk'] == 'P') ? 'Perempuan' : 'Laki-laki';

      $alamat = $this->db('pasien')
        ->where('pasien.no_rkm_medis', $pasien[0]['no_rkm_medis'])
      ...
      $fullAlamat = $alamat['alamat'] . ', ' . $alamat['nm_kel'] . ', ' . $alamat['nm_kec'] . ', ' . $alamat['nm_kab'] . ', ' . $alamat['nm_prop'];
      $pasien[0]['fullAlamat'] = $fullAlamat;

      $tanggal = date('Y-m-d');

      $petugas = $this->core->getUserInfo('fullname', $_SESSION['mlite_user']);

      $kamar = $this->db('kamar')->where('statusdata', '1')->where('status', 'KOSONG')->toArray();

      echo $this->draw('persetujuan.ranap.html', ['pasien' => $pasien, 'tanggal' => $tanggal, 'petugas' => $petugas, 'kamar' => $kamar]);
      exit();
    }
    ...
    public function postMaxAntrian()
    {
      ...
      if($this->settings->get('settings.dokter_ralan_per_dokter') == 'true') {
        ...
        // GET MAX no_reg FROM reg_periksa AND booking_registrasi
        $max_id_periksa = $this->db('reg_periksa')->select(['no_reg' => 'ifnull(MAX(CONVERT(RIGHT(no_reg,3),signed)),0)'])->where('kd_poli', $_POST['kd_poli'])->where('kd_dokter', $_POST['kd_dokter'])->where('tgl_registrasi', date('Y-m-d'))->desc('no_reg')->limit(1)->oneArray();
        $max_id_booking = $this->db('booking_registrasi')->select(['no_reg' => 'ifnull(MAX(CONVERT(RIGHT(no_reg,3),signed)),0)'])->where('kd_poli', $_POST['kd_poli'])->where('kd_dokter', $_POST['kd_dokter'])->where('tanggal_periksa', date('Y-m-d'))->desc('no_reg')->limit(1)->oneArray();
  
        $max_id = MAX($max_id_periksa, $max_id_booking);
      }
...
/*endddddddddddddddddddddddddddddddddd*/
    ...
    public function postSimpanPersetujuanRanap()
    {

      $query = $this->db('mlite_persetujuan_ranap')->save([
        'id' => NULL,
        'nomor_surat' => $_POST['nomor_surat'],
        'no_rawat' => $_POST['no_rawat'],
        'no_rkm_medis' => $_POST['no_rkm_medis'],
        ...
        'alamat' => $_POST['alamat'],
        'tanggal' => $_POST['tanggal'],
        'no_ktp_pj' => $_POST['no_ktp_pj'],
        'nm_pj' => $_POST['nm_pj'],
        'jk_pj' => $_POST['jk_pj'],
        'sts_hubungan' => $_POST['sts_hubungan'],
        'no_tlp_pj' => $_POST['no_tlp_pj'],
        'alamat_pj' => $_POST['alamat_pj'],
        'nm_saksi' => $_POST['nm_saksi'],
        'nm_petugas' => $_POST['nm_petugas'],
        'kamar' => $_POST['kamar'],
        'kelas' => $_POST['kelas'],
        'hak_kls' => $_POST['hak_kls'],

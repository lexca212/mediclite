...
class Admin extends AdminModule
{
    ...
    public function getTriage($no_rawat)
    {
      ...
      echo $this->tpl->draw(MODULES.'/dokter_igd/view/admin/triage.html', true);
      exit();
    }
    ...
    public function getMedisIGD($no_rawat)
    {
      ...
      echo $this->draw('medis.igd.html', [
        ...
        'penilaian_medis_igd' => $this->db('penilaian_medis_igd')->where('no_rawat', revertNoRawat($no_rawat))->join('dokter', 'dokter.kd_dokter=penilaian_medis_igd.kd_dokter')->oneArray(),
    ...
    {
      echo $this->draw('medis.igd.tampil.html', ['penilaian_medis_igd' => $this->db('penilaian_medis_igd')->where('no_rawat', revertNoRawat($no_rawat))->join('dokter', 'dokter.kd_dokter=penilaian_medis_igd.kd_dokter')->oneArray()]);
      exit();
    }
    ...
    public function postMedisIGD()
    {
      ...
      if($this->db('penilaian_medis_igd')->where('no_rawat', $_POST['no_rawat'])->where('kd_dokter', $_POST['kd_dokter'])->oneArray()) {
     ...
     public function getPermintaanRanap($no_rawat)
    {
      $this->assign['kamar'] = $this->db('kamar')
    ->join('bangsal', 'bangsal.kd_bangsal = kamar.kd_bangsal')
    ->where('kamar.statusdata', '1')
    ->where('kamar.status', 'KOSONG')
    ->toArray();

      $dataPermintaanRanap['permintaan_ranap'] = $this->db('permintaan_ranap')->where('no_rawat', revertNoRawat($no_rawat))->oneArray();
      echo $this->draw('form.ranap.html', [
        'rawat_inap' => $this->assign,
        'reg_periksa' => $this->db('reg_periksa')->where('no_rawat', revertNoRawat($no_rawat))->oneArray(),
        'penyakit'  => $this->db('penyakit')->toArray(),
        //'kamar' => $this->db('kamar')->join('bangsal', 'bangsal.kd_bangsal=kamar.kd_bangsal')->where('statusdata', '1')->oneArray(),
        //'penilaian_medis_igd' => $this->db('penilaian_medis_igd')->where('no_rawat', revertNoRawat($no_rawat))->join('dokter', 'dokter.kd_dokter=penilaian_medis_igd.kd_dokter')->oneArray(),
        'pasien'  => $this->db('pasien')->where('no_rawat', revertNoRawat($no_rawat))->join('reg_periksa','pasien.no_rkm_medis=reg_periksa.no_rkm_medis')->oneArray(),
        'dokter'  => $this->db('dokter')->where('no_rawat', revertNoRawat($no_rawat))->join('reg_periksa','dokter.kd_dokter=reg_periksa.kd_dokter')->oneArray(),
        'permintaan_ranap' => $this->db('permintaan_ranap')->where('no_rawat', revertNoRawat($no_rawat))->oneArray(),
        'dataPermintaanRanap' => $dataPermintaanRanap
      ]);
      exit();
    }
    public function getPermintaanRanapTampil($no_rawat)
    {
      echo $this->draw('form.ranap.tampil.html', ['permintaan_ranap' => $this->db('permintaan_ranap')->where('no_rawat', revertNoRawat($no_rawat))->oneArray()]);
      exit();
    }

   public function postPermintaanRanap()
{
    // Jika sudah ada data dengan no_rawat yang sama → update
    if ($this->db('permintaan_ranap')->where('no_rawat', $_POST['no_rawat'])->oneArray()) {
        $update = $this->db('permintaan_ranap')
            ->where('no_rawat', $_POST['no_rawat'])
            ->save([
                'tanggal'   => $_POST['tanggal'],
                'diagnosa'  => $_POST['diagnosa'],
    ...
    // Jika belum ada → insert baru
        ...
        $insert = $this->db('permintaan_ranap')->save([
            'no_rawat'  => $_POST['no_rawat'],
            'tanggal'   => $_POST['tanggal'],
            'diagnosa'  => $_POST['diagnosa'],
            'catatan'   => $_POST['catatan'],
        ...
        // Perbaikan: nama tabel join harus sesuai (bukan `permintaan`)
        $joindata = $this->db('reg_periksa')
            ->join('pasien', 'reg_periksa.no_rkm_medis=pasien.no_rkm_medis')
            ->join('permintaan_ranap', 'reg_periksa.no_rawat=permintaan_ranap.no_rawat')
          	->join('penjab', 'reg_periksa.kd_pj=penjab.kd_pj')
            ->where('reg_periksa.no_rawat', $_POST['no_rawat'])
            ->oneArray();

        if ($joindata) {
            $norawatnya = $joindata['no_rawat'];
            $pasiennya  = $joindata['nm_pasien'];
            $no_rm      = $joindata['no_rkm_medis'];
            $diagnosa   = $joindata['diagnosa'];
          	$catatan	= $joindata['catatan'];
            $tanggal    = $joindata['tanggal'];
          	$penjab		= $joindata['png_jawab'];

            $this->sendToTelegram($norawatnya, $pasiennya, $no_rm, $diagnosa, $catatan, $tanggal, $penjab);
        }
    }
}
    ...
    private function sendToTelegram($norawat, $pasien, $no_rm, $diagnosa, $catatan, $tanggal, $penjab)
{
    ...
    // (opsional) log hasil kirim
    // file_put_contents('telegram_log.txt', $result.PHP_EOL, FILE_APPEND);
}

  //start lembar masuk nicu
  
  public function getLembarMasukNicu($no_rawat)
{
   //$data_lembarNicu['pemeriksaan_ranap'] = $this->db('pemeriksaan_ranap')->where('no_rawat', revertNoRawat($no_rawat))->oneArray();
   //echo $this->draw('lembar.masuk.nicu.html', [
    //'reg_periksa' => $this->db('reg_periksa')->where('no_rawat', revertNoRawat($no_rawat))->oneArray(),
    //'mlite_masuk_nicu' => $this->db('mlite_masuk_nicu')->where('no_rawat', revertNoRawat($no_rawat))->join('dokter', 'dokter.kd_dokter=mlite_masuk_nicu.kd_dokter')->oneArray(),
   // 'pasien'  => $this->db('pasien')->where('no_rawat', revertNoRawat($no_rawat))->join('reg_periksa','pasien.no_rkm_medis=reg_periksa.no_rkm_medis')->oneArray(),
    //'dpjp_ranap' => $this->db('dpjp_ranap')->where('no_rawat', revertNoRawat($no_rawat))->join('dokter','dpjp_ranap.kd_dokter=dokter.kd_dokter')->oneArray(),
   	//'data_lembarNicu' => $data_lembarNicu
   $data_lembarNicu['pemeriksaan_ranap'] = $this->db('pemeriksaan_ranap')->where('no_rawat', revertNoRawat($no_rawat))->oneArray();
      echo $this->draw('lembar.masuk.nicu.html', [
        'reg_periksa' => $this->db('reg_periksa')->where('no_rawat', revertNoRawat($no_rawat))->oneArray(),
        'mlite_masuk_nicu' => $this->db('mlite_masuk_nicu')->where('no_rawat', revertNoRawat($no_rawat))->join('dokter', 'dokter.kd_dokter=mlite_masuk_nicu.kd_dokter')->oneArray(),
 ...
 public function getLembarMasukNicuTampil($no_rawat)
    {
      echo $this->draw('lembar.masuk.nicu.tampil.html', ['mlite_masuk_nicu' => $this->db('mlite_masuk_nicu')->where('no_rawat', revertNoRawat($no_rawat))->oneArray()]);
      exit();
    }

public function postLembarMasukNicu()
    {
      $_POST['kd_dokter'] = $this->core->getUserInfo('username', $_SESSION['mlite_user']);

      if($this->db('mlite_masuk_nicu')->where('no_rawat', $_POST['no_rawat'])->oneArray()) {
        $this->db('mlite_masuk_nicu')
          ->where('no_rawat', $_POST['no_rawat'])
          ->save([
          'kd_dokter'           =>  $_POST['kd_dokter'],
          'tanggal'             =>  $_POST['tanggal'],  

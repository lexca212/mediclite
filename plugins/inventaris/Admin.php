  ...
  public function getPemindahan()
{
    ...
    $inventaris = $this->db('inventaris')
                  ->join('inventaris_barang', 'inventaris_barang.kode_barang=inventaris.kode_barang')
                  ->join('inventaris_ruang', 'inventaris_ruang.id_ruang=inventaris.id_ruang')
                  ->where('status_barang', 'Ada')->toArray();
$pemindahan = $this->db('inventaris_pemindahan')
     ->select('
        inventaris_pemindahan.*,
        ruang_asal.nama_ruang   AS nama_ruang_asal,
        ruang_tujuan.nama_ruang AS nama_ruang_tujuan,
        inventaris.kode_barang,
        inventaris_barang.nama_barang
    ')
    ->join('inventaris_ruang AS ruang_asal', 
           'ruang_asal.id_ruang = inventaris_pemindahan.ruang_asal')
    ->join('inventaris_ruang AS ruang_tujuan', 
           'ruang_tujuan.id_ruang = inventaris_pemindahan.ruang_tujuan')
    ->join('inventaris', 'inventaris.no_inventaris=inventaris_pemindahan.no_inventaris')
       // JOIN ke inventaris_barang
    ->join('inventaris_barang',
        'inventaris_barang.kode_barang = inventaris.kode_barang')
    ->toArray();


    return $this->draw('data.pemindahan.html', [
        'inventaris_ruang' => $inventaris_ruang,
        'inventaris'  => $inventaris,
        'pemindahan'  => $pemindahan
    ]);
}
public function postSavePemindahan()
{
    $no_inventaris = $_POST['no_inventaris'];
    $ruang_tujuan  = $_POST['ruang_tujuan'];
    $tanggal       = $_POST['tanggal_pindah'];
    $keterangan    = $_POST['keterangan'];

    $asalinventaris = $this->db('inventaris')
        ->where('no_inventaris', $no_inventaris)
        ->oneArray();

    if(!$asalinventaris){
        $this->notify('failure', 'Data inventaris tidak ditemukan!');
        return redirect(url([ADMIN,'inventaris','pemindahan']));
    }

    $ruang_asal = $asalinventaris['id_ruang'];

    if($ruang_tujuan == $ruang_asal){
        $this->notify('failure', 'Ruang tidak boleh sama!');
        return redirect(url([ADMIN,'inventaris','pemindahan']));
    }

    $this->db('inventaris_pemindahan')->save([
    ...
    $this->db('inventaris')
        ->where('no_inventaris', $no_inventaris)
        ->update([
            'id_ruang' => $ruang_tujuan,
            'status_barang' => 'Ada'
        ]);

    $this->notify('success', 'Data pemindahan berhasil disimpan!');
    return redirect(url([ADMIN,'inventaris','pemindahan']));
}


public function postGetInventarisByRuang()
{
    header('Content-Type: application/json');

    $id_ruang = $_POST['id_ruang'];

    $data = $this->db('inventaris')
        ->join('inventaris_barang', 'inventaris_barang.kode_barang=inventaris.kode_barang')
        ->where('inventaris.id_ruang', $id_ruang)
        ->toArray();
...
public function postInventarisByRuang()
{
    if(isset($_POST["id_ruang"])){
        ...
        if(count($rows)){
            foreach ($rows as $row) {
                $output .= '<option value="'.$row["no_inventaris"].'">'
                        .$row["nama_barang"].' ('.$row["no_inventaris"].')</option>';
            }
        }

...
class Site extends SiteModule
{
    ...
    public function routes()
    {
        ...
        $this->route('jknmobile/antrian/add', '_getAntreanAddXXX');
        ...
        $this->route('jknmobile/antrian/updatewaktu/(:str)', '_getAntreanUpdateWaktu');
      	...
      	$this->route('jknmobile/antrian/updatewaktu2', '_getAntreanUpdateWaktu2');
        $this->route('jknmobile/antrian/updatewaktu2/(:str)', '_getAntreanUpdateWaktu2');
      
     	$this->route('jknmobile/antrian/updatewaktu3', '_getAntreanUpdateWaktu3');
        $this->route('jknmobile/antrian/updatewaktu3/(:str)', '_getAntreanUpdateWaktu3');
    ...
    public function _resultHapusGagal()
    {
      ...
      */
        ...
        $AddAntrean = $this->db('mlite_antrian_referensi')->where('tanggal_periksa',$date)->where('status_kirim','Gagal')->toArray();

        foreach ($AddAntrean as $value) {
            if ($value['keterangan'] == 'data nohp  belum sesuai.') {
                $bridging_sep = $this->db('bridging_sep')->where('tglsep',$date)->where('nomr',$value['no_rkm_medis'])->oneArray();
                $updatePasien = $this->db('pasien')->where('no_rkm_medis', $value['no_rkm_medis'])->update(['no_tlp' => $bridging_sep['notelep']]);
                if($updatePasien){
                    echo'Berhasil Update No HP !';
                    echo('<br>');
                    echo $value['no_rkm_medis'];
                    echo('<br>');
                    echo $bridging_sep['notelep'];
                    echo('<br>');
                }
            }
        }

        $HapusAdd = $this->db('mlite_antrian_referensi')->where('tanggal_periksa',$date)->where('status_kirim','Gagal')->notlike('keterangan', 'data nik  belum sesuai.')->delete();
        ...
        $HapusUpdateWaktu = $this->db('mlite_antrian_referensi_taskid')->where('tanggal_periksa',$date)->where('status','Gagal')->delete();
    ...
    private function _resultAmbilAntrian()
    {
        ...
        if($header[$this->settings->get('jkn_mobile.header_token')] == false) {
            ...
            $h7 = strtotime('+35 days', strtotime(date('Y-m-d'))) ;
    ...
    public function _getAntreanAddXXX()
    {
        echo '
        <head>
        ...
        if($checkAntrian){
          foreach ($checkAntrian as $q) {
            ...
            $data = [
              ...
              'estimasidilayani' => strtotime($reg_periksa['tgl_registrasi'].' '.$jadwaldokter['jam_mulai']) * 1000,
	...
	public function _getAntreanUpdateWaktu($page = 1)
    {
        ...
        $query = $this->db('mlite_antrian_referensi_taskid')->where('tanggal_periksa',$date)->where('nomor_referensi','!=','')->where('status','Belum')->limit(5)->toArray();
        if (!$query) {
            # code...
            ...
            //         $tindakan_dr = $this->db('rawat_jl_dr')->where('no_rawat', $reg_periksa['no_rawat'])->oneArray();
            //         $tindakan_pr = $this->db('rawat_jl_pr')->where('no_rawat', $reg_periksa['no_rawat'])->oneArray();
            //         $pemeriksaan_ralan = $this->db('pemeriksaan_ralan')->select(['datajam' => 'concat(tgl_perawatan," ",jam_rawat)'])->where('no_rawat', $reg_periksa['no_rawat'])->desc('tgl_perawatan')->oneArray();
            //         $jadwaldokter = $this->db('jadwal')->where('kd_dokter', $reg_periksa['kd_dokter'])->where('kd_poli', $reg_periksa['kd_poli'])->where('hari_kerja', $hari)->oneArray();  
                    
            //         if(!strpos($q['kodebooking'],'MJKN')){
            //             if($reg_periksa['stts_daftar'] == 'Baru'){
            //                 echo 'Sudah waktunya task id 1';
            //                 if($pemeriksaan_ralan){
            //                     echo 'A';
            //                     date_default_timezone_set($this->settings->get('settings.timezone'));
            //                     $tsudah = new \DateTime($pemeriksaan_ralan['datajam']);
            //                     $randnum = rand(5220,5221);
			...
			// echo 'Menjalankan WS taskid (2) mulai tunggu poli<br>';
            // echo '-------------------------------------<br>';

            // foreach ($query as $q) {
            //     if(!$this->db('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('taskid', 2)->oneArray()) {
            //         $pasien = $this->db('pasien')->where('no_peserta', $q['nomor_kartu'])->oneArray();
            //         $q['no_rkm_medis'] = $q['no_rkm_medis'];
            //         if($pasien) {
            //         $q['no_rkm_medis'] = $pasien['no_rkm_medis'];
            //         }
            //         $reg_periksa = $this->db('reg_periksa')->where('tgl_registrasi', $date)->where('no_rkm_medis', $q['no_rkm_medis'])->oneArray();
            //         $sep = $this->db('bridging_sep')->where('no_rawat', $reg_periksa['no_rawat'])->oneArray();  
            //         $tindakan_dr = $this->db('rawat_jl_dr')->where('no_rawat', $reg_periksa['no_rawat'])->oneArray();
            //         $tindakan_pr = $this->db('rawat_jl_pr')->where('no_rawat', $reg_periksa['no_rawat'])->oneArray();
            //         $pemeriksaan_ralan = $this->db('pemeriksaan_ralan')->select(['datajam' => 'concat(tgl_perawatan," ",jam_rawat)'])->where('no_rawat', $reg_periksa['no_rawat'])->desc('tgl_perawatan')->oneArray();
                    
            //         if(!strpos($q['kodebooking'],'MJKN')){
            //             if($tindakan_dr || $tindakan_pr){

            //                 date_default_timezone_set($this->settings->get('settings.timezone'));
            //                 $task = $this->db('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 1)->oneArray();

            //                 $timestamp = $task['waktu'];
            //                 $timestamp = $timestamp/1000;
            //                 $jam = date('Y-m-d H:i:s', $timestamp);
            //                 $tsudah = new \DateTime($jam);
            ...
            foreach ($query as $q) {
                if(!$this->db('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('taskid', 3)->oneArray()) {
                    ...
                    if($tindakan_dr || $tindakan_pr){
                        if($reg_periksa['stts_daftar'] == 'Baru'){
                            if(!strpos($q['kodebooking'],'MJKN')){
                                echo 'Sudah waktunya task id 3';

                                date_default_timezone_set($this->settings->get('settings.timezone'));
                                $task = $this->db('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 2)->oneArray();
                            
                                $timestamp = $task['waktu'];
                                $timestamp = $timestamp/1000;
                                $jam = date('Y-m-d H:i:s', $timestamp);
                                $tsudah = new \DateTime($jam);
                                $randnum = rand(120,300);
                                $tsudah->modify('+ '.$randnum.' seconds');
                                $diterima = $tsudah->format('Y-m-d H:i:s');
                            ...
                            echo $date;
                            echo '<br>';
                            echo $jadwaldokter['jam_mulai'];
                            echo '<br>';
                            echo $reg_periksa['kd_dokter'];
                            echo '<br>';
                            echo $reg_periksa['kd_poli'];
                            echo '<br>';
                            echo $hari;
                            echo '<br>';
                            $reg_periksa = $this->db('reg_periksa')->where('tgl_registrasi', $date)->where('no_rkm_medis', $q['no_rkm_medis'])->oneArray();
                            $sep = $this->db('bridging_sep')->where('no_rawat', $reg_periksa['no_rawat'])->oneArray();  
                            date_default_timezone_set($this->settings->get('settings.timezone'));
                            $datajam =  $date.' '.$jadwaldokter['jam_mulai'];
                            $tsudah = new \DateTime($datajam);
                            $randnum = rand(3000,3100);
                            $tsudah->modify('- '.$randnum.' seconds');
                            $diterima = $tsudah->format('Y-m-d H:i:s');
                            $this->db('mlite_antrian_referensi_taskid')
                                ->save([
            ...
            foreach ($query as $q) {
                if(!$this->db('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 4)->oneArray()) {
                    ...
                    if($tindakan_dr || $tindakan_pr){
                      echo 'Sudah waktunya task id 4';
					  date_default_timezone_set($this->settings->get('settings.timezone'));
                      $task = $this->db('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 3)->oneArray();

                      $timestamp = $task['waktu'];
                      $timestamp = $timestamp/1000;
                      $jam = date('Y-m-d H:i:s', $timestamp);
                      $tsudah = new \DateTime($jam);
                      $randnum = rand(900,1200);
                      $tsudah->modify('+ '.$randnum.' seconds');
                      $diterima = $tsudah->format('Y-m-d H:i:s');
            ...
            foreach ($query as $q) {
              if(!$this->db('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 5)->oneArray()) {
                ...
                if($tindakan_dr || $tindakan_pr){ 
                    ...
                    echo 'Sudah waktunya task id 5';

                  	date_default_timezone_set($this->settings->get('settings.timezone'));
                    $task = $this->db('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 4)->oneArray();

                    $timestamp = $task['waktu'];
                    $timestamp = $timestamp/1000;
                    $jam = date('Y-m-d H:i:s', $timestamp);
                    $tsudah = new \DateTime($jam);
                    $randnum = rand(600,700);
                    $tsudah->modify('+ '.$randnum.' seconds');
                    $diterima = $tsudah->format('Y-m-d H:i:s');
            ...
            foreach ($query as $q) {
                if(!$this->db('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 6)->oneArray()) {
                    $reg_periksa = $this->db('reg_periksa')->where('tgl_registrasi', $date)->where('no_rkm_medis', $q['no_rkm_medis'])->oneArray();
                    $sep = $this->db('bridging_sep')->where('no_rawat', $reg_periksa['no_rawat'])->oneArray();  
                    $resep_obat = $this->db('resep_obat')->select(['datajam' => 'concat(tgl_peresepan," ",jam_peresepan)'])->where('no_rawat', $reg_periksa['no_rawat'])->desc('tgl_peresepan')->oneArray();
                    $tindakan_dr = $this->db('rawat_jl_dr')->where('no_rawat', $reg_periksa['no_rawat'])->oneArray();
                    $tindakan_pr = $this->db('rawat_jl_pr')->where('no_rawat', $reg_periksa['no_rawat'])->oneArray();
                    ...
                    if($tindakan_dr || $tindakan_pr){   
                        if($resep_obat){
                            ...
                            date_default_timezone_set($this->settings->get('settings.timezone'));
                            $task5 = $this->db('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 5)->oneArray();

                            $timestamp = $task5['waktu'];
                            $timestamp = $timestamp/1000;
                            $jam = date('Y-m-d H:i:s', $timestamp);
                            $tsudah = new \DateTime($jam);
                            $randnum = rand(600,700);
            ...
            foreach ($query as $q) {
                if(!$this->db('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 7)->oneArray()) {
                    $reg_periksa = $this->db('reg_periksa')->where('tgl_registrasi', $date)->where('no_rkm_medis', $q['no_rkm_medis'])->oneArray();
                    $sep = $this->db('bridging_sep')->where('no_rawat', $reg_periksa['no_rawat'])->oneArray();  
                    $resep_obat = $this->db('resep_obat')->select(['datajam' => 'concat(tgl_perawatan," ",jam)'])->where('no_rawat', $reg_periksa['no_rawat'])->where('concat(tgl_perawatan," ",jam)', '<>', 'concat(tgl_peresepan," ",jam_peresepan)')->desc('tgl_peresepan')->oneArray();
                  	$tindakan_dr = $this->db('rawat_jl_dr')->where('no_rawat', $reg_periksa['no_rawat'])->oneArray();
                    $tindakan_pr = $this->db('rawat_jl_pr')->where('no_rawat', $reg_periksa['no_rawat'])->oneArray();
                    ...
                    if($tindakan_dr || $tindakan_pr){   
                        if($resep_obat){
                            ...
                            date_default_timezone_set($this->settings->get('settings.timezone'));
                            $task5 = $this->db('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 6)->oneArray();
    ...
    //Kirim Uupdate waktu setelah jadwal dokter selesai
    public function _getAntreanUpdateWaktu2($page = 1)
    {
        echo '
        <head>
        <meta http-equiv="refresh" content="60">
        ...
        if (!$query) {
            ...
            // echo 'Menjalankan WS taskid (1) mulai tunggu poli<br>';
            // echo '-------------------------------------<br>';

            // foreach ($query as $q) {
            //     if(!$this->db('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('taskid', 1)->oneArray()) {
            //         $pasien = $this->db('pasien')->where('no_peserta', $q['nomor_kartu'])->oneArray();
            //         $q['no_rkm_medis'] = $q['no_rkm_medis'];
            //         if($pasien) {
            //         $q['no_rkm_medis'] = $pasien['no_rkm_medis'];
            ...
            //         if(!strpos($q['kodebooking'],'MJKN')){
            //             if($jam > $jadwaldokter['jam_selesai']){
            //                 if($reg_periksa['stts_daftar'] == 'Baru'){
            //                 echo 'Sudah waktunya task id 1';
            //                 if($pemeriksaan_ralan){
            //                     echo 'A';
            ...
            //             }
            //             } else {
            //                 echo 'Skip dulu, belum selesai jadwalnya';
            //             }
            //         }
            //     }
            // }
            ...
            echo 'Menjalankan WS taskid (4) permintaan resep poli<br>';
            echo '-------------------------------------<br>';
            ...
            echo 'Menjalankan WS taskid (5) permintaan resep poli<br>';
            echo '-------------------------------------<br>';
            ...
            foreach ($query as $q) {
                if(!$this->db('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 6)->oneArray()) {
                    $reg_periksa = $this->db('reg_periksa')->where('tgl_registrasi', $date)->where('no_rkm_medis', $q['no_rkm_medis'])->oneArray();
                    $sep = $this->db('bridging_sep')->where('no_rawat', $reg_periksa['no_rawat'])->oneArray();  
                    $resep_obat = $this->db('resep_obat')->where('no_rawat', $reg_periksa['no_rawat'])->oneArray();
                    if(!empty($resep_obat)){
                        echo 'A';

                        date_default_timezone_set($this->settings->get('settings.timezone'));
                        $task = $this->db('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 5)->oneArray();
            ...
            $sql = "SELECT * FROM mlite_antrian_referensi WHERE kodebooking NOT IN (SELECT nomor_referensi FROM mlite_antrian_referensi_taskid WHERE tanggal_periksa = '{$date}' AND taskid IN ('7')) AND tanggal_periksa = '{$date}'  AND status_kirim = 'Sudah' LIMIT 100";
            $stmt = $this->db()->pdo()->prepare($sql);
            $stmt->execute();
            $query = $stmt->fetchAll();

            echo 'Menjalankan WS taskid (7) permintaan resep poli<br>';
            echo '-------------------------------------<br>';
            ...
            foreach ($query as $q) {
                if(!$this->db('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 7)->oneArray()) {
                    ...
                    if($resep_obat){
                        ...
                        date_default_timezone_set($this->settings->get('settings.timezone'));
                        $task = $this->db('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 6)->oneArray();
    ...
    //Kirim update waktu free roam, buat kirim ulang
    public function _getAntreanUpdateWaktu3($page = 1)
    {
        echo '
        <head>
                <meta http-equiv="refresh" content="60">
        ...
        if (!$query) {
            ...
            $sql = "SELECT * FROM mlite_antrian_referensi WHERE kodebooking NOT IN (SELECT nomor_referensi FROM mlite_antrian_referensi_taskid WHERE tanggal_periksa = '{$date}' AND taskid IN ('6')) AND tanggal_periksa = '{$date}'  AND status_kirim = 'Sudah' LIMIT 100";
            $stmt = $this->db()->pdo()->prepare($sql);
            $stmt->execute();
            $query = $stmt->fetchAll();

...
class Admin extends AdminModule
{
    ...
    public function navigation()
    {
        return [
            ...
            'Pengaturan' => 'settings',
          	'Add Antrean' => 'addantrean',
            'Update Waktu' => 'updatetaskid',
        ];
    }
    ...
    public function getManage()
    {
      $sub_modules = [
        ...
        ['name' => 'Add Antrean', 'url' => url([ADMIN, 'jkn_mobile', 'addantrean']), 'icon' => 'tasks', 'desc' => 'Add Antrean'],
        ['name' => 'Update Waktu', 'url' => url([ADMIN, 'jkn_mobile', 'updatetaskid']), 'icon' => 'tasks', 'desc' => 'Update Waktu'],
      ];
    ...
    // ================ Add Update Waktu Antrean ================
    public function getUpdateTaskId($page = 1)
    {
        ...
        // pagination
        $totalRecords = $this->db('mlite_antrian_referensi_taskid')
            ->select('nomor_referensi')
            ->where('tanggal_periksa',$date)
            //->where('tanggal_periksa','2024-07-16')
            ->where('status','Gagal')
            ->toArray();

        $pagination = new \Systems\Lib\Pagination($page, count($totalRecords), 10, url([ADMIN, 'jkn_mobile', 'updatetaskid', '%d']));
        $this->assign['pagination'] = $pagination->nav('pagination', '5');
        $this->assign['totalRecords'] = $totalRecords;

        // list
        $offset = $pagination->offset();
        $rows = $this->db('mlite_antrian_referensi_taskid')
            ->where('status','Gagal')
            ->where('tanggal_periksa',$date)
            //->where('tanggal_periksa','2024-07-16')
            ->like('nomor_referensi', '%' . $phrase . '%')
            ->offset($offset)
            ->limit($perpage)
            ->toArray();
        ...
        if (count($rows)) {
            foreach ($rows as $row) {
                ...
                $row['editURL'] = url([ADMIN, 'jkn_mobile', 'updatetaskidedit', $row['nomor_referensi'], $row['taskid']]);
                $row['delURL']  = url([ADMIN, 'jkn_mobile', 'updatetaskiddelete', $row['nomor_referensi'], $row['taskid']]);
                $this->assign['list'][] = $row;
            }
        }

        $this->assign['addURL'] = url([ADMIN, 'jkn_mobile', 'updatetaskidadd']);

        return $this->draw('updatetaskid.manage.html', ['updatetaskid' => $this->assign]);
    }

    public function getUpdateTaskIdAdd()
    {
        $this->_addHeaderFiles();
    ...
    public function getUpdateTaskIdEdit($id, $waktu)
    {
        $this->_addHeaderFiles();
        $row = $this->db('mlite_antrian_referensi_taskid')->where('nomor_referensi',$id)->where('taskid',$waktu)->oneArray();
        if (!empty($row)) {
            $this->assign['form'] = $row;
            $this->assign['title'] = 'Edit Update Waktu';

            return $this->draw('updatetaskid.form.html', ['updatetaskid' => $this->assign]);
        } else {
            redirect(url([ADMIN, 'jkn_mobile', 'updatetaskid']));
        }
    }

    public function getUpdateTaskIdDelete($id)
    {
        if ($this->db('mlite_antrian_referensi_taskid')->where('nomor_referensi',$id)->delete()) {
    ...
    public function postUpdateTaskIdSave($id = null)
    {
        $errors = 0;

        if (!$id) {
            $location = url([ADMIN, 'jkn_mobile', 'updatetaskidadd']);
        } else {
            $location = url([ADMIN, 'jkn_mobile', 'updatetaskid']);
        }

        // if (checkEmptyFields(['updatetaskid','nomor_referensi'], $_POST)) {
        ...
        if (!$errors) {
            ...
            if (!$id) {    // new
                $query = $this->db('mlite_antrian_referensi_taskid')->save($_POST);
            } else {        // edit
                $query = $this->db('mlite_antrian_referensi_taskid')->where('nomor_referensi', $id)->save(['waktu'=>$_POST['waktu'],'status'=>$_POST['status']]);
            }
    ...
    // ================ End Update Waktu Antrean ================

    // ================ Add Antrean ================
    public function getAddAntrean($page = 1)
    {
        $this->_addHeaderFiles();
        $perpage = '10';
        $date = date('Y-m-d');
        ...
        $hari=$day[$tentukan_hari];
      	$jadwaldokter = $this->db('jadwal')->where('hari_kerja', $hari)->desc('jam_selesai')->notLike('kd_poli','ANT')->oneArray();
        ...
        // list
        $sql = "SELECT mlite_antrian_referensi.* FROM mlite_antrian_referensi JOIN bridging_sep ON mlite_antrian_referensi.no_rkm_medis = REPLACE(bridging_sep.nomr,'.','') WHERE mlite_antrian_referensi.tanggal_periksa = '$date' AND bridging_sep.tglsep = '$date' AND bridging_sep.jnspelayanan = '2' AND mlite_antrian_referensi.status_kirim = 'Sudah'";
        $query = $this->db()->pdo()->prepare($sql);
        $query->execute();
        $antrian_salah = $query->fetchAll();

        $sql = "SELECT * FROM bridging_sep WHERE tglsep = '$date' AND jnspelayanan = '2' AND kdpolitujuan NOT IN ('-','ANT','GIG','GIZ','HDL','IGDK','KIA','LAB','OBT','RAD','UMU','IGD')";
        $query = $this->db()->pdo()->prepare($sql);
        $query->execute();
        $sep_terbit = $query->fetchAll();
      
        $sep_no_rujukan = array_column($sep_terbit, 'no_rujukan');
        $sep_no_skdp = array_column($sep_terbit, 'noskdp');

        if (count($antrian_salah)) {
            foreach ($antrian_salah as $value) {
              if (!in_array($value['nomor_referensi'], $sep_no_rujukan) && !in_array($value['nomor_referensi'], $sep_no_skdp)) {
                $value = htmlspecialchars_array($value);
                $value['editURL'] = url([ADMIN, 'jkn_mobile', 'addantreanedit', $value['kodebooking']]);
                $value['delURL']  = url([ADMIN, 'jkn_mobile', 'addantreandelete', $value['kodebooking'], $value['nomor_referensi']]);
        ...
        $this->assign['addURL'] = url([ADMIN, 'jkn_mobile', 'addantreanadd']);

        return $this->draw('addantrean.manage.html', ['addantrean' => $this->assign, 'jadwaldokter' => $jadwaldokter]);
    }

    // ================ HAPUS ANTREAN ================
    public function postHapusAntrean()
    {
        $date = date('Y-m-d');
        $hapus_ref = $this->db('mlite_antrian_referensi')->where('tanggal_periksa', $date)->where('status_kirim','Gagal')->delete();
      exit();
    }
    // ================ END HAPUS ANTRIAN ================
  
    // ================ HAPUS WAKTU ================
    public function postHapusWaktu()
    {
        $date = date('Y-m-d');
        $hapus_ref = $this->db('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('status','Gagal')->delete();
      exit();
    }
    // ================ END HAPUS WAKTU ================

    public function getAddAntreanAdd()
    {
        $this->_addHeaderFiles();
        if (!empty($redirectData = getRedirectData())) {
            $this->assign['form'] = filter_var_array($redirectData, FILTER_SANITIZE_STRING);
    ...
    public function getAddAntreanEdit($id)
    {
        $this->_addHeaderFiles();
        $row = $this->db('mlite_antrian_referensi')->where('kodebooking',$id)->oneArray();
        $sep = $this->db('bridging_sep')->where('nomr',$row['no_rkm_medis'])->where('tglsep',$row['tanggal_periksa'])->oneArray();
        if (!empty($row)) {
            $this->assign['form'] = $row;
            $this->assign['sep'] = $sep;
            $this->assign['title'] = 'Edit Add Antrean';

            return $this->draw('addantrean.form.html', ['addantrean' => $this->assign]);
        } else {
            redirect(url([ADMIN, 'jkn_mobile', 'addantrean']));
        }
    }

    public function getAddAntreanDelete($id,$no_referensi)
    {
        if ($this->db('mlite_antrian_referensi')->where('kodebooking',$id)->where('nomor_referensi',$no_referensi)->delete()) {
            $this->notify('success', 'Hapus sukses');
        } else {
            $this->notify('failure', 'Hapus gagal');
        }
    ...
    public function postAddAntreanSave($id = null)
    {
        $errors = 0;

        if (!$id) {
            $location = url([ADMIN, 'jkn_mobile', 'addantreanadd']);
        } else {
            $location = url([ADMIN, 'jkn_mobile', 'addantrean']);
        }

        // if (checkEmptyFields(['tanggal_periksa','no_rkm_medis','kodebooking','id'], $_POST)) {
        //     $this->notify('failure', 'Isian kosong');
        //     redirect($location, $_POST);
        // }
        ...
        if (!$errors) {
            ...
            if (!$id) {    // new
                $query = $this->db('mlite_antrian_referensi')->save($_POST);
            } else {        // edit
                $query = $this->db('mlite_antrian_referensi')->where('kodebooking', $id)->save(['status_kirim'=>$_POST['status_kirim'], 'nomor_referensi' => $_POST['referensi']]);
            }
    ...
    public function getQrAntrol(){
        ...
        $sql = "SELECT * FROM bridging_sep WHERE tglsep = '$tanggal' AND jnspelayanan = '2' AND kdpolitujuan NOT IN ('IGDK','HDL')";
        $query = $this->db()->pdo()->prepare($sql);
        $query->execute();
        $sep_terbit = $query->fetchAll();
        $jml_sep = 0;
        ...
        foreach ($sep_terbit as $valuex) {
            $nomor_referensi = '';
              //$cari_antrol = $this->db('mlite_antrian_referensi')->where('tanggal_periksa',$tanggal)->where('kodebooking','!=','')->where('status_kirim','Sudah')->oneArray();
          	$cari_antrol = $this->db('mlite_antrian_referensi')->where('tanggal_periksa',$tanggal)->where('nomor_referensi',$valuex['no_rujukan'])->where('kodebooking','!=','')->where('status_kirim','Sudah')->oneArray();
            if (!$cari_antrol) {
                if ($valuex['noskdp'] != '') {
                    $cari_antrol_kontrol = $this->db('mlite_antrian_referensi')->where('tanggal_periksa',$tanggal)->where('nomor_referensi',$valuex['noskdp'])->where('kodebooking','!=','')->where('status_kirim','Sudah')->oneArray();
					//$cari_antrol_kontrol = $this->db('mlite_antrian_referensi')->where('tanggal_periksa',$tanggal)->where('kodebooking','!=','')->where('status_kirim','Sudah')->oneArray();
          	...
          	$sep = $valuex['no_sep'];
          	$sql = "SELECT DISTINCT nomor_referensi, taskid FROM mlite_antrian_referensi_taskid WHERE nomor_referensi = '$nomor_referensi' AND status = 'Sudah'";
            $query = $this->db()->pdo()->prepare($sql);
            $query->execute();
            $task = $query->fetchAll();
    ...
    {
        $this->_addHeaderFiles();
        return $this->draw('bookingantrol.html', ['row' => $this->db('mlite_antrian_referensi')->join('pasien','mlite_antrian_referensi.no_rkm_medis=pasien.no_rkm_medis')->toArray()]);
    }

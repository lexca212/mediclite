...
class Site extends SiteModule
{
        ...
        if (!$query) {
            ...
            foreach ($query as $q) {
                if(!$this->core->mysql('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 1)->oneArray()) {
                    ...
                    $tindakan_dr = $this->core->mysql('rawat_jl_dr')->where('no_rawat', $reg_periksa['no_rawat'])->oneArray();
                    $tindakan_pr = $this->core->mysql('rawat_jl_pr')->where('no_rawat', $reg_periksa['no_rawat'])->oneArray();
            ...
            foreach ($query as $q) {
                if(!$this->core->mysql('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('taskid', 2)->oneArray()) {
                    $pasien = $this->core->mysql('pasien')->where('no_peserta', $q['nomor_kartu'])->oneArray();
                    $q['no_rkm_medis'] = $q['no_rkm_medis'];
                    if($pasien) {
                    $q['no_rkm_medis'] = $pasien['no_rkm_medis'];
                    ...
                    if(!strpos($q['kodebooking'],'MJKN')){
                        if($tindakan_dr || $tindakan_pr){
                            ...
                            date_default_timezone_set($this->settings->get('settings.timezone'));
                            $task = $this->core->mysql('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 1)->oneArray();
                    ...
                    if($tindakan_dr || $tindakan_pr){
                        if($reg_periksa['stts_daftar'] == 'Baru'){
                            if(!strpos($q['kodebooking'],'MJKN')){
                                ...
                                date_default_timezone_set($this->settings->get('settings.timezone'));
                                $task = $this->core->mysql('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 2)->oneArray();
            ...
            foreach ($query as $q) {
                if(!$this->core->mysql('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 4)->oneArray()) {
                    ...
                    if($tindakan_dr || $tindakan_pr){
					  ...
					  date_default_timezone_set($this->settings->get('settings.timezone'));
                      $task = $this->core->mysql('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 3)->oneArray();
              ...
              if(!$this->core->mysql('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 5)->oneArray()) {
                ...
                if($tindakan_dr || $tindakan_pr){ 
                    ...
                    echo 'Sudah waktunya task id 5';
                  	...
                  	date_default_timezone_set($this->settings->get('settings.timezone'));
                    $task = $this->core->mysql('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 4)->oneArray();
                ...
                if(!$this->core->mysql('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 6)->oneArray()) {
                    ...
                    if($tindakan_dr || $tindakan_pr){   
                        if($resep_obat){
                            ...
                            date_default_timezone_set($this->settings->get('settings.timezone'));
                            $task5 = $this->core->mysql('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 5)->oneArray();
                ...
                if(!$this->core->mysql('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 7)->oneArray()) {
                    ...
                    if($tindakan_dr || $tindakan_pr){   
                        if($resep_obat){
                            ...
                            date_default_timezone_set($this->settings->get('settings.timezone'));
                            $task5 = $this->core->mysql('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 6)->oneArray();
    ...
    public function _getAntreanUpdateWaktu2($page = 1)
    {
        ...
        if (!$query) {
            ...
            foreach ($query as $q) {
                if(!$this->core->mysql('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('taskid', 1)->oneArray()) {
                    $pasien = $this->core->mysql('pasien')->where('no_peserta', $q['nomor_kartu'])->oneArray();
                    $q['no_rkm_medis'] = $q['no_rkm_medis'];
                    if($pasien) {
                    $q['no_rkm_medis'] = $pasien['no_rkm_medis'];
            ...
            foreach ($query as $q) {
                if(!$this->core->mysql('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 6)->oneArray()) {
                    $reg_periksa = $this->core->mysql('reg_periksa')->where('tgl_registrasi', $date)->where('no_rkm_medis', $q['no_rkm_medis'])->oneArray();
                    $sep = $this->core->mysql('bridging_sep')->where('no_rawat', $reg_periksa['no_rawat'])->oneArray();  
                    $resep_obat = $this->core->mysql('resep_obat')->where('no_rawat', $reg_periksa['no_rawat'])->oneArray();
                    if(!empty($resep_obat)){
                        echo 'A';

                        date_default_timezone_set($this->settings->get('settings.timezone'));
                        $task = $this->core->mysql('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 5)->oneArray();
            ...
            foreach ($query as $q) {
                if(!$this->core->mysql('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 7)->oneArray()) {
                    ...
                    if($resep_obat){
                        ...
                        date_default_timezone_set($this->settings->get('settings.timezone'));
                        $task = $this->core->mysql('mlite_antrian_referensi_taskid')->where('tanggal_periksa', $date)->where('nomor_referensi', $q['kodebooking'])->where('status','Sudah')->where('taskid', 6)->oneArray();

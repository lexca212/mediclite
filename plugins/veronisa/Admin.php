...
class Admin extends AdminModule
{
  ...
  public function anyIndex($page = 1)
  {
    ...
    if (count($rows)) {
      foreach ($rows as $row) {
        ...
        // Cek apakah data sudah ada di tabel mlite_apotek_online_resep_response_log
        $resep_online_sep_data_exists = $this->db('mlite_apotek_online_sep_data')
          ->where('no_rawat', $row['no_rawat'])
          ->oneArray();
        $row['resep_online_sep_data_exists'] = $resep_online_sep_data_exists;
  ...
  {    
    ...
    $obat_data = $this->db()->pdo()->prepare("
      ...
      JOIN mlite_apotek_online_maping_obat maom ON rd.kode_brng = maom.kode_brng 
  ...
  public function postKirimApotikOnline()
  {
    ...
    try {
      ...
      if (isset($json_resep['response'])) {
        ...
        echo json_encode([
          'success' => true,
          'message' => 'Data resep berhasil dikirim ke BPJS',
        ...
        foreach ($_POST['racikan'] as $index => $racikan) {
          ...
          if (isset($racikan['detail']) && is_array($racikan['detail'])) {
            foreach ($racikan['detail'] as $detail_index => $detail) {
              if (preg_match('/([\d\.,]+)\s*SK/i', $detail['nama_obat_bpjs'], $match)) {
    			  $sk = str_replace(',', '.', $match[1]); // ubah koma jadi titik
                  $sk = (float)$sk; // ubah ke angka desimal
              } else {
                  $sk = 0; // kalau tidak ditemukan
              }
              try {
                $racikan_data = [
                  'NOSJP' => $response['noApotik'],
                  ...
                  'PERMINTAAN' => (int)$sk,
                  'JMLOBT' => (((int)$racikan['SIGNA1RACIKAN']*(int)$racikan['SIGNA2RACIKAN'])*(int)$racikan['JHORACIKAN']),

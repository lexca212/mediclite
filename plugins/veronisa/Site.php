...
class Site extends SiteModule
{
    ...
    {
      	$this->bpjsurl                 = $this->settings->get('veronisa.bpjs_api_url');
    ...
    public function routes()
    {
        ...
        $this->route('vero/logout', function () {
      	...
      	$this->route('vero/riwayat/(:str)/(:str)/(:str)', '_getRiwayatObat');
      	$this->route('vero/riwayat/save', '_getRiwayatObatSave');
      	$this->route('vero/riwayat/sep/(:str)', '_getRiwayatObatSEP');
    }
  
    public function _getRiwayatObatSEP()
    {
        $slug = parseURL();
        ...
        $url = $this->bpjsurl.'pelayanan/obat/daftar/'.$sep;
      	echo 'REAL URL : '.$url;
        ...
        if (!empty($stringDecrypt)) {
          ...
          echo '<br><br>';
          echo json_encode(json_decode($decompress), JSON_PRETTY_PRINT);
          echo '<br><br>';
  ...
  public function _getRiwayatObatSave()
  {
      $slug = parseURL();
      date_default_timezone_set('UTC');
      ...
      $klaim_vero = $this->core->mysql('mlite_veronisa')
          ->join('bridging_sep', 'bridging_sep.no_sep = mlite_veronisa.nosep')
          ->where('mlite_veronisa.tanggal', '2026-01-02')
          ->limit(10)
          ->toArray();

      foreach ($klaim_vero as $value) {
          $tgl_awal  = $value['tglsep'];
          $tgl_akhir = $value['tglsep'];
          $nokartu   = $value['no_kartu'];

          $url    = $this->bpjsurl . 'riwayatobat/' . $tgl_awal . '/' . $tgl_akhir . '/' . $nokartu;
          $output = BpjsService::get($url, null, $this->consid, $this->secretkey, $this->user_key, null);
          echo 'output : '.$output.'<br>';
          $json = json_decode($output, true);
          if (!isset($json['response'])) {
              error_log("Gagal fetch riwayatobat untuk noKartu {$nokartu}");
              continue;
          }
          ...
          // pastikan ada minimal 1 history
          if (empty($data['list']['history']) || !is_array($data['list']['history'])) {
              continue;
          }

          // ambil 1 history saja
          $h = $data['list']['history'][0];

          /* ---------- skip bila sudah tersimpan ---------- */
          $exists = $this->db('mlite_ref_apotek_online_kronis')
                          ->where('no_rawat', $value['no_rawat'])
                          ->where('nosjp', $h['nosjp'])
                          ->count();
          if ($exists) {
              echo $value['no_rawat'] . ' - ' . $h['nosjp'] . ' => sudah ada, skip<br>';
              continue;
          }

          /* ---------- simpan ---------- */
          $ok = $this->db('mlite_ref_apotek_online_kronis')
                    ->save([
          ...
          echo $value['no_rawat'] . ' - ' . $h['nosjp'] . ' - ' . $h['tglpelayanan'] . ' => ';
          echo $ok ? 'Berhasil Simpan ref kronis' : 'Gagal Simpan ref kronis';
          echo '<br>';
      }
      exit();
  }
  
    public function _getRiwayatObat()
    {
        $slug = parseURL();
        date_default_timezone_set('UTC');
        $tStamp = strval(time() - strtotime("1970-01-01 00:00:00"));

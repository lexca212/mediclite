...
class Admin extends AdminModule
{
    ...
    {
      if (file_exists(WEBAPPS_PATH.'/berkasrawat/'.$_POST['lokasi_file'])) {
        $hapus = unlink(WEBAPPS_PATH.'/berkasrawat/'.$_POST['lokasi_file']);
        if($hapus) {
    ...
    public function getRiwayatPerawatan($no_rkm_medis)
    {
      ...
      $riwayat['pasien'] = $this->core->db('pasien')->where('no_rkm_medis', $no_rkm_medis)->oneArray();
      $reg_periksa = $this->db('reg_periksa')
    ->join('poliklinik', 'poliklinik.kd_poli=reg_periksa.kd_poli')
    ->join('dokter AS d1', 'd1.kd_dokter=reg_periksa.kd_dokter')
    ->leftJoin('dpjp_ranap', 'dpjp_ranap.no_rawat=reg_periksa.no_rawat')
    ->leftJoin('dokter AS d2', 'd2.kd_dokter=dpjp_ranap.kd_dokter')
    ->join('penjab', 'penjab.kd_pj=reg_periksa.kd_pj')
    ->where('no_rkm_medis', $no_rkm_medis)
    ->desc('tgl_registrasi')
    ->select([
        ...
        'poliklinik.nm_poli',
        'penjab.png_jawab',
        'COALESCE(d2.nm_dokter, d1.nm_dokter) AS nama_dokter' // Jika ada di dpjp_ranap, gunakan, jika tidak pakai dari reg_periksa
    ])
    ->toArray();
      ...
      foreach ($reg_periksa as $row) {
        ...
        // =========================== PENILAIAN AWAL MEDIS RALAN KANDUNGAN ===========================
        $row['penilaian_medis_ralan_kandungan'] = $this->core->db('penilaian_medis_ralan_kandungan')
        ->join('dokter', 'penilaian_medis_ralan_kandungan.kd_dokter=dokter.kd_dokter')
        ->where('no_rawat', $row['no_rawat'])
        ->oneArray();

        // =========================== PENILAIAN AWAL MEDIS IGD ===========================
        $row['penilaian_medis_igd'] = $this->core->db('penilaian_medis_igd')
        ->join('dokter', 'penilaian_medis_igd.kd_dokter=dokter.kd_dokter')
        ->where('no_rawat', $row['no_rawat'])
        ->oneArray();

        // =========================== PENILAIAN AWAL MEDIS RALAN UMUM ===========================
        $row['penilaian_medis_ralan'] = $this->core->db('penilaian_medis_ralan')
        ->join('dokter', 'penilaian_medis_ralan.kd_dokter=dokter.kd_dokter')
        ->where('no_rawat', $row['no_rawat'])
        ->oneArray();

        // =========================== PENILAIAN AWAL MEDIS RALAN BAYI / ANAK ===========================
        $row['penilaian_medis_ralan_anak'] = $this->core->db('penilaian_medis_ralan_anak')
        ->join('dokter', 'penilaian_medis_ralan_anak.kd_dokter=dokter.kd_dokter')
        ->where('no_rawat', $row['no_rawat'])
        ->oneArray();

        // =========================== PENILAIAN AWAL MEDIS RALAN BEDAH ===========================
        $row['penilaian_medis_ralan_bedah'] = $this->core->db('penilaian_medis_ralan_bedah')
        ->join('dokter', 'penilaian_medis_ralan_bedah.kd_dokter=dokter.kd_dokter')
        ->where('no_rawat', $row['no_rawat'])
        ->oneArray();

        // =========================== PENILAIAN AWAL MEDIS RALAN MATA ===========================
        $row['penilaian_medis_ralan_mata'] = $this->core->db('penilaian_medis_ralan_mata')
        ->join('dokter', 'penilaian_medis_ralan_mata.kd_dokter=dokter.kd_dokter')
        ->where('no_rawat', $row['no_rawat'])
        ->oneArray();

        // =========================== PENILAIAN AWAL MEDIS RALAN ORTHOPEDI ===========================
        $row['penilaian_medis_ralan_orthopedi'] = $this->core->db('penilaian_medis_ralan_orthopedi')
        ->join('dokter', 'penilaian_medis_ralan_orthopedi.kd_dokter=dokter.kd_dokter')
        ->where('no_rawat', $row['no_rawat'])
        ->oneArray();

        // =========================== PENILAIAN AWAL MEDIS RALAN PARU ===========================
        $row['penilaian_medis_ralan_paru'] = $this->core->db('penilaian_medis_ralan_paru')
        ->join('dokter', 'penilaian_medis_ralan_paru.kd_dokter=dokter.kd_dokter')
        ->where('no_rawat', $row['no_rawat'])
        ->oneArray();

        // =========================== PENILAIAN AWAL MEDIS RALAN THT ===========================
        $row['penilaian_medis_ralan_tht'] = $this->core->db('penilaian_medis_ralan_tht')
        ->join('dokter', 'penilaian_medis_ralan_tht.kd_dokter=dokter.kd_dokter')
        ->where('no_rawat', $row['no_rawat'])
        ->oneArray();

        // =========================== SKRINING GIZI ===========================
        $row['skrining_gizi'] = $this->core->db('skrining_gizi')
        ->join('pegawai', 'skrining_gizi.nip=pegawai.nik')
        ->where('no_rawat', $row['no_rawat'])
        ->toArray();

        // =========================== ASUHAN GIZI ===========================
        $row['asuhan_gizi'] = $this->core->db('asuhan_gizi')
        ->join('pegawai', 'asuhan_gizi.nip=pegawai.nik')
        ->where('no_rawat', $row['no_rawat'])
        ->toArray();

        // =========================== RESUME PASIEN ===========================
        $row['resume_pasien'] = $this->core->db('resume_pasien')
        ->join('dokter', 'resume_pasien.kd_dokter=dokter.kd_dokter')
        ->where('no_rawat', $row['no_rawat'])
        ->oneArray();

        // =========================== RESUME PASIEN RANAP ===========================
        $row['resume_pasien_ranap'] = $this->core->db('resume_pasien_ranap')
        ->join('dokter', 'resume_pasien_ranap.kd_dokter=dokter.kd_dokter')
        ->where('no_rawat', $row['no_rawat'])
        ->oneArray();
        
         // ===========================HASIL PEMERIKSAAN USG ==========================
        $row['hasil_pemeriksaan_usg'] = $this->core->db('hasil_pemeriksaan_usg')
        ->join('dokter', 'hasil_pemeriksaan_usg.kd_dokter=dokter.kd_dokter')
        ->where('no_rawat', $row['no_rawat'])
        ->oneArray();
        // ===========================Lembar Masuk Nicu==========================
        $row['mlite_masuk_nicu'] = $this->core->db('mlite_masuk_nicu')
        ->join('dokter', 'mlite_masuk_nicu.kd_dokter=dokter.kd_dokter')
        ->where('no_rawat', $row['no_rawat'])
        ->oneArray();

        // =========================== LAPORAN OPERASI ===========================
        $row['laporan_operasi'] = $this->core->db('laporan_operasi')
        ->where('no_rawat', $row['no_rawat'])
        ->group('no_rawat')
        ->group('tanggal')
        ->asc('tanggal')
        ...
        $data_triase_igd = $this->core->db('data_triase_igd')
          ->join('master_triase_macam_kasus', 'data_triase_igd.kode_kasus=master_triase_macam_kasus.kode_kasus')
          ->where('no_rawat', $row['no_rawat'])
          ->oneArray();

        $data_triase_igd_sekunder = $this->core->db('data_triase_igdsekunder')
          ->where('no_rawat', $row['no_rawat'])
          ->oneArray();

        $data_triase_igd_primer = $this->core->db('data_triase_igdprimer')
          ->where('no_rawat', $row['no_rawat'])
          ->oneArray();

        $data_triase_igddetail_skala1= $this->core->db('data_triase_igddetail_skala1')
        ...
        $data_triase_igddetail_skala2= $this->core->db('data_triase_igddetail_skala2')
        ...
        $data_triase_igddetail_skala3= $this->core->db('data_triase_igddetail_skala3')
        ...
        $data_triase_igddetail_skala4= $this->core->db('data_triase_igddetail_skala4')
        ...
        $data_triase_igddetail_skala5= $this->core->db('data_triase_igddetail_skala5')
        ...
        $row['data_triase_igddetail_skala1']= $data_triase_igddetail_skala1;
        $row['data_triase_igddetail_skala2']= $data_triase_igddetail_skala2;
        $row['data_triase_igddetail_skala3']= $data_triase_igddetail_skala3;
        $row['data_triase_igddetail_skala4']= $data_triase_igddetail_skala4;
        $row['data_triase_igddetail_skala5']= $data_triase_igddetail_skala5;
        $row['data_triase_igd_sekunder']= $data_triase_igd_sekunder;
        $row['data_triase_igd_primer']= $data_triase_igd_primer;
        $row['data_triase_igd']= $data_triase_igd;

...
abstract class Main
{
    ...
    public function getPasienInfo($field, $no_rkm_medis)
    {
        ...
        //$row = $this->db()->pdo()->prepare("SELECT * FROM pasien WHERE no_rkm_medis = '$no_rkm_medis'");
      	return $row[$field];
    }
    ...
    public function setNoReg($kd_dokter, $kd_poli = null)
    {
        ...
        if($this->settings->get('settings.dokter_ralan_per_dokter') == 'true') {
          ...
          $max_id_periksa = $this->db('reg_periksa')->select(['no_reg' => 'ifnull(MAX(CONVERT(RIGHT(no_reg,3),signed)),0)'])->where('kd_poli', $kd_poli)->where('kd_dokter', $kd_dokter)->where('tgl_registrasi', date('Y-m-d'))->desc('no_reg')->limit(1)->oneArray();
          $max_id_booking = $this->db('booking_registrasi')->select(['no_reg' => 'ifnull(MAX(CONVERT(RIGHT(no_reg,3),signed)),0)'])->where('kd_poli', $kd_poli)->where('kd_dokter', $kd_dokter)->where('tanggal_periksa', date('Y-m-d'))->desc('no_reg')->limit(1)->oneArray();

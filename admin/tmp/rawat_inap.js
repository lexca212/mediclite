...
$("#display").on("click", ".update_plafon", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();

  var no_rawat = $(this).attr("data-no_rawat");

  var url = baseURL + '/rawat_inap/kiriminacbgs?t=' + mlite.token;
...
$("#display").on("click", ".sep", function(event){
  ...
  var url = baseURL + '/vclaim/bynokartu/' + no_peserta + '/2026-02-25?t=' + mlite.token;
  ...
  bootbox.confirm("Apakah Anda yakin ingin menghapus data ini?", function(result){
    ...
    if (result){
      ...
      $.post(url, {
        ...
        $('input:text[name=jam_rawat]').val("11:40:54");
        $('#notif').html("<div class=\"alert alert-danger alert-dismissible fade in\" role=\"alert\" style=\"border-radius:0px;margin-top:-15px;\">"+

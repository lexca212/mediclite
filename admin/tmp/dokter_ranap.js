  ...
  bootbox.confirm("Apakah Anda yakin ingin menghapus data ini?", function(result){
    ...
    if (result){
      ...
      $.post(url, {
        ...
        $('input:text[name=tgl_perawatan]').val("2026-04-17");
        $('input:text[name=tgl_registrasi]').val("2026-04-17");
        $('input:text[name=jam_rawat]').val("11:03:07");
...
$(document).ready(function () {
  ...
  $('.databarang_ajax').selectator({
    ...
    load: function (search, callback) {
      ...
      $.ajax({
        url: 'http://localhost/mediclite/admin/dokter_ranap/ajax?show=databarang&nama_brng=' + encodeURIComponent(search) + '&t=85577f627f59',
        type: 'GET',
        dataType: 'json',

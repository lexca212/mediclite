...
$("#form").on("click", "#simpan", function(event){
  ...
  if(no_rawat == '') {
    ...
    $.post(url,{
      ...
      var audio = new Audio('http://localhost/mediclite/assets/sound/' + data.status + '.mp3');
      audio.play();
  ...
  bootbox.confirm("Apakah Anda yakin ingin menghapus data ini?", function(result){
    ...
    if (result){
      ...
      $.post(url, {
        ...
        $('input:text[name=tgl_perawatan]').val("2026-02-27");
        $('input:text[name=tgl_registrasi]').val("2026-02-27");
        $('input:text[name=jam_rawat]').val("10:54:34");
        $('#notif').html("<div class=\"alert alert-danger alert-dismissible fade in\" role=\"alert\" style=\"border-radius:0px;margin-top:-15px;\">"+
...
$("#display").on("click",".panggil", function(event){
    ...
    var pola = /(Tn|Ny|By)\.?\s*/gi; 

    var nm_pasien_bersih = nm_pasien.replace(pola, function(match, p1) {
        var prefix = p1.toLowerCase(); 
    ...
    var text = nm_pasien_bersih + ", nomor antrian " + no_reg + ", ke " + nm_poli;
